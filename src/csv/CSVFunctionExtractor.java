...
// csv --> ast
import csv.csv2ast.CSVAST;
import csv.csv2ast.CSV2AST;
...
public class CSVFunctionExtractor {
    ...
    Queue<CSVAST> csvFifoQueue = new LinkedList<CSVAST>(); // create a csv_ast queue to save a ast for a function
    ...
    public FunctionDefBase getNextFunction() throws IOException, InvalidCSVFile{
        ...
        if (!csvFifoQueue.isEmpty())
        {
            CSVAST csv_ast = csvFifoQueue.remove();
            function = csv2ast.convert(csv_ast);
        }
        return function;
    }
    ...
    private void addEdgeRowsUntilNextFile() throws InvalidCSVFile
    {
        ...
        while( relReader.hasNextRow())
        {
            KeyedCSVRow currEdgeRow = relReader.getNextRow();
            ...
            if( !currType.equals(PHPCSVEdgeTypes.TYPE_AST_PARENT_OF))
                throw new InvalidCSVFile( "edgeReader, line " + relReader.getCurrentLineNumber() + ": "
